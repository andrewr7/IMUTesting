...
void setup() 
{
  ...
  String min = String.valueOf(mn);
  String hou = String.valueOf(h);  
  String datetime = mon+"-"+day+"-"+yea+"_"+hou+"-"+min+"-"+sec;
  println(datetime);
  output = createWriter("angles_"+datetime+".txt"); 
  output2 = createWriter("mostrecenttime.txt"); 
  output2.println(datetime);
  ...
  if (myPort.available() >= 18) {
    ...
    if (inputString != null && inputString.length() > 0) {
      ...
      if (inputStringArr.length >= 5) { // q1,q2,q3,q4,\r\n so we have 5 elements
        ...
        q[4] = decodeFloat(inputStringArr[4]);
        q[5] = decodeFloat(inputStringArr[5]);
        q[6] = decodeFloat(inputStringArr[6]);
        //q[7] = decodeFloat(inputStringArr[13]);
        //q[8] = decodeFloat(inputStringArr[14]);
        //q[9] = decodeFloat(inputStringArr[15]);
...
void draw() {
  ...
  output.println(millis() + " " + degrees(Euler[0])  + " " + degrees(Euler[1]) + " " + degrees(Euler[2])+" " + q[4]+" " + q[5]+" " + q[6]); // Write the coordinate to the file
}
...
void keyPressed() {
  if (key == 'h') {
    ...
    println("pressed x: file is stopped");
